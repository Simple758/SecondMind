...
object OfflineNLP{
  ...
  fun summarize(t:String,m:Int=3):String{
    val s=t.split(Regex("(?<=[.!?])\\s+")).filter{it.isNotBlank()}
    if(s.size<=m)return t.trim()
  ...
  fun sentiment(t:String):Int{
    val p=listOf("gain","rally","beat","record","up","bull","positive","growth","strong","mejora","sube")
    val n=listOf("fall","drop","miss","down","bear","negative","decline","weak","crash","cae","baja")
    var sc=0; val x=t.lowercase()
    p.forEach{ if(x.contains(it)) sc++ }; n.forEach{ if(x.contains(it)) sc-- }
    return sc.coerceIn(-3,3)
  }
}
